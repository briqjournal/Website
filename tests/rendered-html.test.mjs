...
test("publishes every Volume 3 Issue 3 canonical record as localized bilingual HTML", async () => {
  const issueArticles = archive.articles.filter((article) => article.volume === 3 && article.issue === 3);
  assert.equal(issueArticles.length, 12);
  ...
  for (const article of issueArticles) {
    ...
    assert.equal(trResponse.status, 200, `TR v03-i03 ${slug}`);
    assert.equal(enResponse.status, 200, `EN v03-i03 ${slug}`);
    const [trHtml, enHtml] = await Promise.all([trResponse.text(), enResponse.text()]);
    assert.match(trHtml, /<section class="article-fulltext" id="tam-metin"><h2>Tam Metin<\/h2>/, `TR full text ${slug}`);
    assert.match(enHtml, /<section class="article-fulltext" id="full-text-body"><h2>Full Text<\/h2>/, `EN full text ${slug}`);
    assert.match(trHtml, /class="article-body-section"/, `TR body ${slug}`);
...
test("publishes every Volume 3 Issue 1 canonical record as localized bilingual HTML", async () => {
  const issueArticles = archive.articles.filter((article) => article.volume === 3 && article.issue === 1);
  assert.equal(issueArticles.length, 14);
  for (const article of issueArticles) {
    const slug = article.slug;
    const [trResponse, enResponse] = await Promise.all([
      renderPath(`/tr/makaleler/${slug}`),
    ...
    assert.equal(trResponse.status, 200, `TR v03-i01 ${slug}`);
    assert.equal(enResponse.status, 200, `EN v03-i01 ${slug}`);
    const [trHtml, enHtml] = await Promise.all([trResponse.text(), enResponse.text()]);
    assert.match(trHtml, /<section class="article-fulltext" id="tam-metin"><h2>Tam Metin<\/h2>/, `TR full text ${slug}`);
    assert.match(enHtml, /<section class="article-fulltext" id="full-text-body"><h2>Full Text<\/h2>/, `EN full text ${slug}`);
    assert.match(trHtml, /class="article-body-section"/, `TR body ${slug}`);
